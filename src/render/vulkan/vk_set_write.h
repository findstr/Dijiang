...
public:
	static vk_set_write &inst() {
		static vk_set_write inst;
		return inst;
	};
public:
	vk_set_write &write(VkDescriptorSet desc_set, int dst_binding, int dst_array_element, VkDescriptorBufferInfo &info) {
	...
	vk_set_write &write(VkDescriptorSet desc_set, int dst_binding, int dst_array_element, VkDescriptorImageInfo &info) {
		write_cmds.emplace_back();
		auto &cmd = write_cmds.back();
